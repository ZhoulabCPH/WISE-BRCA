...
def get_args(lr, wd):
    ...
    parser.add_argument('--checkpoint-path',
                        default='./checkpoint/WISE-BRCA/', type=Path,
...
def train(lr, wd):
    ...
    for fold, (train_index, validation_index) in enumerate(skf.split(list(slides_train_validation.iloc[:, 0]), labels)):
        ...
        workspace_train = slides_train_validation.iloc[train_index]
        workspace_validation = slides_train_validation.iloc[validation_index]
        workspace_test = pd.read_csv(args.workspace_test)
        ...
        data_train = BC(workspace_train, f'{args.slides_path_224}', f'{args.slides_path_512}')
        data_train_loader = DataLoader(data_train, args.batch_size, shuffle=True, num_workers=4, drop_last=True,
                                       collate_fn=collate)
        data_validation = BC(workspace_validation, f'{args.slides_path_224}', f'{args.slides_path_512}')
        ...
        data_test = BC(workspace_test,  f'{args.slides_dir_224}{args.aug}', f'{args.slides_dir_512}{args.aug}')
        ...
        model = WISE_BRCA().cuda()
        ckpt = torch.load(
            rf'{args.checkpoint_path}/checkpoint_max_validation_auc_{fold}.pth',
            map_location='cuda:0')
        ...
        auc_HMUCH, data_HMUCH = calculate_metrics(model, data_HMUCH)


        print(
            f'Fold {fold} Epoch: {fold} train AUC: {auc_train}, validation AUC: {auc_validation}, test AUC: {auc_test}, YYH AUC: {auc_YYH},'
...
if __name__ == '__main__':
    ...
    sys.stdout = open('F:\Yangzijian\project\BC\CCT_\checkpoint\Mossformer/log.txt', 'w')
    lr = 1e-5
    wd = 5e-4
    train(lr, wd)